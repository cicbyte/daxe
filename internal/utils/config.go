...
func (c *Config) GetAppDir() string {
	...
	c.AppDir = c.GetAppSeriesDir() + "/daxe"
	return c.AppDir
}
...
func (c *Config) GetLogPath() string {
	if c.LogPath == "" {
		...
		c.LogPath = filepath.Join(c.GetLogDir(), fmt.Sprintf("daxe_log_%s.log", now))
	}
	return c.LogPath
}
...
func GetDefaultConfig() *models.AppConfig {
	...
	// PicGo默认配置
	config.PicGo.Server = "http://127.0.0.1:36677"
	config.PicGo.Timeout = 30
